...
const FeatureProducts = () => {
  ...
  const [selectedSpecs, setSelectedSpecs] = useState(null); // State for modal data
  const [activeTab, setActiveTab] = useState("specs"); // State for modal tabs
  const navigate = useNavigate();

  // 1. Fetch Featured Products from Laravel API
  ...
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <p className="mt-2 text-muted">Loading featured vehicles...</p>
      </div>
    );
  }

  return (
    <section className="section-featured py-4 py-md-5 bg-light">
      <div className="container">
        <h2 className="fw-bold mb-4 px-2">Featured Inventory</h2>
          ...
          {products.map((product) => {
            ...
            return (
              <div className="col-lg-3 col-md-4 col-6" key={product.id}>
                <div className="card h-100 border-0 shadow-sm hover-up position-relative overflow-hidden">
                  {/* RIBBON */}
                  ...
                  >
                    <img
                      ...
                      onError={(e) => {
                        e.target.src =
                          "https://placehold.co/400x300?text=Vehicle";
                      }}
                    />
                  </div>
                  ...
                  <div className="card-body p-2 p-md-3 d-flex flex-column">
                    ...
                    <div className="mt-auto pt-2 border-top">
                      <button
                        className="btn btn-sm btn-outline-primary w-100 d-flex align-items-center justify-content-center gap-2 py-2"
                        onClick={() => {
                          setSelectedSpecs(product);
                          setActiveTab("specs");
                        }}
                      >
                        Specifications <FaPlusCircle />
                      </button>
                    </div>
                  </div>
                </div>
